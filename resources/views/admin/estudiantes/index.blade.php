...
@endsection
	...
	<script>
		...
		$(document).ready(function(){
			$('#content-load-data').load("{{ route('admin.estudiantes.async', 'main') }}", function(){
			});

			$('body').on('click', '.add', function(event){
				event.preventDefault();
				$('#content-load-data').load("{{ route('admin.estudiantes.async', 'create') }}", function(){
				});
			});
			...
			$('body').on('submit', '.form-add', function(event){
				...
				$.ajax({
                    type:"POST",
                    url:"{{ route('admin.estudiantes.async', 'store') }}",
                    dataType:"json",
                    data:$(this).serialize(),
				    success:function(response){
                    	if($.isEmptyObject(response.error)){
	                    	...
	                    	setTimeout(function() {
	                    		$('.message-sucess').hide();
	                            $('#content-load-data').load("{{ route('admin.estudiantes.async', 'main') }}");
	                        }, 3000);
						} else {
